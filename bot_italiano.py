...
class SuperYouTubeDownloader:
    ...
    async def get_playlist_info(self, url):
        """Ottiene informazioni della playlist"""
        ydl_opts = {
            'quiet': True,
            'no_warnings': True,
            'extract_flat': True,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                if 'entries' in info:
                    return {
                        'title': info.get('title', 'Playlist Sconosciuta'),
                        'uploader': info.get('uploader', 'Sconosciuto'),
                        'entries': [entry for entry in info['entries'] if entry][:10],  # Limita a 10 video
                        'total_entries': len(info['entries'])
                    }
                return None
            except Exception as e:
                logger.error(f"Errore nel recupero info playlist: {e}")
                return None
    ...
    async def download_audio(self, url, user_id, format='mp3', quality='192'):
        ...
        try:
            # Ottieni prima le info del video
            info = await self.get_video_info(url)
            if not info:
                return None, "Impossibile ottenere le informazioni del video. Controlla l'URL."
            
            # Controlla durata (max 30 minuti per utenti premium)
            ...
            # Crea directory download specifica per utente
            user_dir = os.path.join(DOWNLOAD_PATH, str(user_id))
            os.makedirs(user_dir, exist_ok=True)
            
            # Configura opzioni yt-dlp con impostazioni avanzate
    ...
    async def download_playlist(self, url, user_id, format='mp3', quality='192'):
        """Download intera playlist"""
        playlist_info = await self.get_playlist_info(url)
        if not playlist_info:
            return None, "Impossibile ottenere le informazioni della playlist."
        
        if len(playlist_info['entries']) > 5:
            return None, "Playlist troppo grande (max 5 video)."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Messaggio di benvenuto con info utente
    welcome_message = f"""
...
"""
    ...
    if update.message:
        ...
        await update.callback_query.edit_message_text(playlist_text, parse_mode=ParseMode.MARKDOWN)
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_playlist:
        ...
        processing_msg = await update.message.reply_text(
            "🔍 Analizzando playlist... Attendi per favore..."
        )
        
        playlist_info = await downloader.get_playlist_info(url)
        if not playlist_info:
            await processing_msg.edit_text("❌ Impossibile ottenere le informazioni della playlist. Controlla l'URL.")
            return
        
        info_text = f"""
🎶 **Playlist: {playlist_info['title']}**
👤 **Canale:** {playlist_info['uploader']}
📊 **Video:** {len(playlist_info['entries'])} (mostrando i primi 5)
🔗 **URL:** {url}
...
"""
        ...
        # Pulisci il titolo per evitare errori di parsing
        clean_title = info['title'].replace('*', '').replace('_', '').replace('`', '')
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gestisci callback download playlist
        ...
        if len(parts) >= 6:
            ...
            await query.edit_message_text(
                f"⬇️ Scaricando playlist come {format_type.upper()} {quality}k...\nQuesto potrebbe richiedere diversi minuti..."
            )
            ...
            if results:
                ...
                await query.edit_message_text(
                    f"✅ Download playlist completato! {len(results)} file sono stati inviati."
                )
            else:
                await query.edit_message_text(f"❌ Download playlist fallito: {playlist_info}")
